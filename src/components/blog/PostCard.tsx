...
export default function PostCard({ post }: PostCardProps) {
	return (
		<div className="group block h-full hover:-translate-y-1 transition-all duration-500 ease-in-out">
			<Link href={`/posts/${post.id}`} className="h-full">
				<Card className="h-full flex flex-col animate-fade-in p-6 hover:bg-gradient-to-r from-blue-50 to-purple-50 dark:bg-slate-800/50 backdrop-blur-md rounded-2xl shadow-xl border border-gray-200 hover:shadow-lg hover:shadow-indigo-100 group-hover:border-indigo-500 group-hover:bg-indigo-50/50 transition-all duration-500 ease-in-out overflow-hidden">
					...
					{/* Card Content - Make this flex-grow to take remaining space */}
